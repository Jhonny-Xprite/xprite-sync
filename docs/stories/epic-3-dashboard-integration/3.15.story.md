...
- [spec-3.15-requirements.json](story-3.15/specs/spec-3.15-requirements.json) — Phase 1: Requirements gathered
- [spec-3.15-complexity.json](story-3.15/specs/spec-3.15-complexity.json) — Phase 2: Complexity assessment (STANDARD)
- [spec-3.15-specification.md](story-3.15/specs/spec-3.15-specification.md) — Phase 3: Formal specification
- [spec-3.15-critique.json](story-3.15/specs/spec-3.15-critique.json) — Phase 4: QA validation (APPROVED ✅)

**Implementation Plan:**
- [implementation.yaml](story-3.15/implementation.yaml) — Detailed execution plan with phases & subtasks
...
### Modified
- `packages/api/src/services/memory.ts`
  - Scan `C:\Users\[user]\.claude\projects\[project]/memory/` directory
  - Parse memory files (YAML frontmatter + content)
  - Support search/filtering by agent, type, keyword

- `packages/dashboard/src/components/memory/MemoryBrowser.tsx`
  - Display memory file tree
  - Show memory list with filters
  - No hardcoded demo memories

- `packages/dashboard/src/components/memory/MemoryViewer.tsx`
  - Display selected memory file content
  - Show frontmatter (type, agent, description)
  - Show "No memories yet" if directory empty

- `packages/dashboard/src/hooks/useMemories.ts`
  - Use `/api/memory/` endpoints
  - Handle search functionality

### Unchanged
- `packages/api/src/index.ts` (routes exist)
- Memory file structure (already working)
