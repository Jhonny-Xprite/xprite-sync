...
- [spec-3.13-requirements.json](story-3.13/specs/spec-3.13-requirements.json) — Phase 1: Requirements gathered
- [spec-3.13-complexity.json](story-3.13/specs/spec-3.13-complexity.json) — Phase 2: Complexity assessment (COMPLEX)
- [spec-3.13-specification.md](story-3.13/specs/spec-3.13-specification.md) — Phase 3: Formal specification
- [spec-3.13-critique.json](story-3.13/specs/spec-3.13-critique.json) — Phase 4: QA validation (APPROVED ✅)

**Implementation Plan:**
- [implementation.yaml](story-3.13/implementation.yaml) — Detailed execution plan with phases & subtasks
...
### Modified
- `packages/api/src/services/engine.ts`
  - Replace hardcoded values with real system metrics
  - Use SystemMetricsCollector for honest data
  - Add engine_available flag

- `packages/api/src/services/workflows.ts`
  - Scan `.aiox-core/development/workflows/*.yaml` files
  - Return real workflow definitions (not hardcoded)
  - Handle missing files gracefully

- `packages/api/src/services/tasks.ts`
  - (if exists) Return real task data or empty state

- `packages/dashboard/src/components/engine/EngineWorkspace.tsx`
  - Use `useEngine()` hook for real data
  - Show engine_available flag truthfully
  - Remove mock status indicators

- `packages/dashboard/src/components/workflows/WorkflowCatalog.tsx`
  - Display real workflows from API
